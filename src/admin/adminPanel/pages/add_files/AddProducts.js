    ...
    if( file.type == "application/vnd.ms-excel"){ setFileType(true)
        if (e.target.files) {
            ...
            reader.onload = (e) => {
                ...
                const x = json.map(({product_name,product_price,quantity,branch}) => [product_name,product_price,quantity,branch]);
                setArray(x)
                setDataLength(json.length)
                let isKeyPresent = json.some(el => {
                    if( el.hasOwnProperty('product_name','product_price','quantity','branch') ){setFileType(true)}
                    if( !el.hasOwnProperty('product_name','product_price','quantity','branch') ){setFileType(false)}
                })
            };
...
const SubmitFile = (e)=>{
    ...
    if(array.length>0){
        ...
        setLoading(true)
            ...
            axios.post(`https://app-31958949-9c59-4302-94ca-f9eaf62903af.cleverapps.io/api/add-finite-products-2`,body)
            .then(response =>{
                setArray('')
                setFileName('')
...
const defaultOptions = {
  ...
  return (
    <div className="dashboard_center_content_container">
        <div className="Add_file_header"> اضافة منتجات بلا حدود</div>
