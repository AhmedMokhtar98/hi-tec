...
export default function OutBuyings() {
...
const SubmitSearch = ()=>{
    ...
    if(auth != 'admin'){
        const data = {search:search, searchType:searchType, branch:branchname, offset:(currentPage-1)*10}
            axios.post(`https://app-31958949-9c59-4302-94ca-f9eaf62903af.cleverapps.io/api/out-buyings-view`,data,{
            headers:{"x-access-token":localStorage.getItem('token')}
            }).then((response)=>{
                setLoading(false)
        ...
        axios.post(`https://app-31958949-9c59-4302-94ca-f9eaf62903af.cleverapps.io/api/admin-outbuyings-view`,data,{
            headers:{"x-access-token":localStorage.getItem('token')}
            }).then((response)=>{
            setLoading(false)
            setData(response.data.result);
    ...
    return (
        ...
        <div className="Page_Container">
            <div className="Page_Header"> 
                <GoBack/>
                {!matches1 && <HeaderMenu/>}
                <div className="Header_word">المنتجات الخارجية</div>
            </div>
                        ...
                        <tr className="table_tr_head">
                            ...
                            {Data.map((item, i)=>(
                                <tr className="table_tr buyings_tr" key={i+1}  >  
                                    ...
                                    <td className="table_td buyings_td">{item.code}</td>
